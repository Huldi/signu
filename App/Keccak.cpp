...
const uint32_t KeccakRounds = 24;
...
const uint32_t StateSize = 1600 / (8 * 8);
const uint32_t MaxBlockSize = 200 - 2 * (224 / 8);
const uint16_t m_bits = 256;

/// block size (less or equal to MaxBlockSize)
const uint16_t m_blockSize = (200 - 2 * (256 / 8));
...
{
	keccak_reset();
	keccak_add(data, numBytes);

	for (int j = 0; j < 4; j++)
	{
		for (int k = 0; k < 8; k++)
			hash[j * 8 + k] = (unsigned char)(m_hash[j] >> (8 * k));
	}
}
